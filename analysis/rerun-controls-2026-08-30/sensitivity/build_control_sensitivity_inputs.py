#!/usr/bin/env python3
"""Patched copy (analysis/rerun-controls-2026-08-30): --expected-profiles replaces the
hard-coded 217 so the Trip 5 + Trip 4 filtered table (312 profiles) can be used.
...
# Trip 5 profiles are V<site>..., Trip 4 profiles e####_S<site>...; the site key keeps
# the prefix so that Trip 4 and Trip 5 visits of the same site are not paired.
SAMPLE_RE = re.compile(
    r"^(?:e\d+_)?(?P<prefix>[VS])(?P<site>\d+)(?P<position>PR|D|S)r(?P<replicate>\d+)"
)
...
def parse_profile(profile_id: str) -> tuple[str, str]:
    match = SAMPLE_RE.match(profile_id)
    if match is None:
        raise ValueError(f"mapped profile has an unparseable ID: {profile_id}")
    return match.group("prefix") + match.group("site"), POSITION[match.group("position")]


def main() -> int:
    root = Path(__file__).resolve().parents[3]  # patched copy sits one level deeper
    ...
    parser.add_argument("--expected-profiles", type=int, default=217)
    ...
    # per-trip screens: a candidate is removed only from the profiles of its own
    # screen, so the completeness check below is done per screen
    candidates_by_screen: dict[str, set[str]] = {}
    for row in calls:
        candidates_by_screen.setdefault(row.get("screen", "Trip5"), set()).add(row["feature_id"])
    ...
    if len(samples) != args.expected_profiles:
        raise ValueError(f"expected {args.expected_profiles} mapped profiles, found {len(samples)}")
    ...
    for chunk in chunks:
        ...
        for screen, ids in candidates_by_screen.items():
            cols = np.asarray([removal[s].get("trip", "Trip5") == screen for s in samples])
            hit = chunk.index.isin(ids)
            if hit.any() and cols.any() and (chunk.to_numpy(dtype=np.float64)[hit][:, cols] > 0).any():
                raise ValueError(f"{screen} candidate contaminant ASVs remain in {screen} profiles")
    ...
    # (global candidate check replaced by the per-screen check in the chunk loop)
