#!/usr/bin/env bash
set -euo pipefail
# Patched copy (analysis/rerun-controls-2026-08-30): sensitivity inputs come from the
# per-trip rerun (Trip 5 + Trip 4 screens) instead of analysis/v3/control_audit.

project_root=${1:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}
output_root=${2:-"$project_root/analysis/v3/control_sensitivity"}
python_bin=${PYTHON:-python3}
rerun_root=${RERUN_ROOT:?set RERUN_ROOT to the rerun directory holding outputs/}
expected_profiles=${EXPECTED_PROFILES:-312}
input_root="$output_root/sensitivity_inputs"
...
run_step build_inputs \
  "$python_bin" "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/build_control_sensitivity_inputs.py" \
  --filtered-table "$rerun_root/outputs/mapped_feature_table_control_filtered.tsv.gz" \
  --calls "$rerun_root/outputs/primary_contaminant_calls.tsv" \
  --removal "$rerun_root/outputs/removal_fraction_by_profile.tsv" \
